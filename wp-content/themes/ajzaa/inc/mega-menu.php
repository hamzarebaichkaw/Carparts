...
if ( !function_exists( 'ajzaa_add_mega_menu_support' ) ):

	function ajzaa_add_mega_menu_support() {
			...
			add_filter( 'wp_setup_nav_menu_item', 'ajzaa_add_custom_nav_fields' );

			add_action( 'wp_update_nav_menu_item', 'ajzaa_update_custom_nav_fields', 10, 3 );

			add_filter( 'wp_edit_nav_menu_walker', 'ajzaa_edit_menu_walker', 10, 2 );
...
add_action( 'init', 'ajzaa_add_mega_menu_support' );



/* Add custom fields to menu */

if ( !function_exists( 'ajzaa_add_custom_nav_fields' ) ):

	function ajzaa_add_custom_nav_fields( $menu_item ) {

		
		$menu_item->mega_menu_icon = get_post_meta( $menu_item->ID, '_ajzaa_mega_menu_icon', true )? get_post_meta( $menu_item->ID, '_ajzaa_mega_menu_icon', true ) : '';
		
		$menu_item->mega_menu_bg_image = get_post_meta( $menu_item->ID, '_ajzaa_mega_menu_bg_image', true )? get_post_meta( $menu_item->ID, '_ajzaa_mega_menu_bg_image', true ) : '';
		
		$menu_item->mega_menu = get_post_meta( $menu_item->ID, '_ajzaa_mega_menu', true ) ? 1 : 0;
...
if ( !function_exists( 'ajzaa_update_custom_nav_fields' ) ):

	function ajzaa_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {


			$value = isset($_REQUEST['menu-item-mega-menu-icon'][$menu_item_db_id])? $_REQUEST['menu-item-mega-menu-icon'][$menu_item_db_id] : '' ;
			update_post_meta( $menu_item_db_id, '_ajzaa_mega_menu_icon', $value );
			
			
			$value = isset($_REQUEST['menu-item-mega-menu-bg-image'][$menu_item_db_id])? $_REQUEST['menu-item-mega-menu-bg-image'][$menu_item_db_id] : '' ;
			update_post_meta( $menu_item_db_id, '_ajzaa_mega_menu_bg_image', $value );
		

			$value = isset( $_REQUEST['menu-item-mega-menu'][$menu_item_db_id] ) ? 1 : 0;
			update_post_meta( $menu_item_db_id, '_ajzaa_mega_menu', $value );
...
if ( !function_exists( 'ajzaa_edit_menu_walker' ) ):

	function ajzaa_edit_menu_walker( $walker, $menu_id ) {



		class ajzaa_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit {
				...
				$mega_menu_html = '';
  				...
  				$icons = unserialize( $icons );
					...
					$mega_menu_html .= '<p class="field-custom description description-wide">
						...
						$mega_menu_html .= "<option vlaue='".esc_attr($value) ."' ". esc_attr($selected).">".esc_attr($value)."</option>";
						
						}
		        		
		        	$mega_menu_html .= '</select>

		                '.esc_html__( 'display icone', 'ajzaa' ).'</label>
					...
					$mega_menu_html .= '<p class="field-custom description description-wide">
			                ...
			                '.esc_html__( 'Mega Menu (Background image)', 'ajzaa' ).'
					...
					$mega_menu_html .= '<p class="field-custom description description-wide">
		                ...
		                '.esc_html__( 'Mega Menu (classic)', 'ajzaa' ).'
		...
		return 'ajzaa_Walker_Nav_Menu_Edit';
